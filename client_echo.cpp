	...
	while(loop_invariant == 0){
		...
		switch(choice){
			...
			case 5: cout<<"Current working directory is: "<<endl;
					...
					cout<<recieve(file_descriptor,25);
					break;
			case 6: cout<<"Enter file name"<<endl;
					cin>>file_name;
					send(file_descriptor,string("touch")+" "+file_name,256);
					break;
			case 9: loop_invariant = 1;
					break;
		}
	}
}

void telnet_sum(int file_descriptor){
int n = -11;
string no;
int p =0;
stringstream stream;
...
while(p==0){
   	...
   	if(n<0){
   		stream << n;
		string no = stream.str();					
   		send(file_descriptor,no,256);
   		break;   						
   		}
	stream << n;
	string no = stream.str();					
	...
	cout<<"The sum is : "<<recieve(file_descriptor,256)<<endl;
}
	...
	if(file_descriptor < 0){
		cout<<"Error, failed to create sockets"<<endl;
   			...
   			if(status < 0){
   				...
   				cin>>name;
   				int n = send(file_descriptor,string("hello from ")+name,20);
   				if(n<0){
   					cout<<"error in client"<<endl;
   				}
   				else{
   					cout<<recieve(file_descriptor,20);
   				}
